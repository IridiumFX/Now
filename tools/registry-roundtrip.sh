...
#
# keygen -> package -> sign -> publish -> procure -> verify -> unpack ->
# compile and link against the fetched dependency, plus the two negatives
# that matter: a tampered artifact must be refused, and --locked must
# refuse a drifted descriptor without touching the network.
#
